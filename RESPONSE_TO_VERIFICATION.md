...
- 1st strategy stayed flat because the enhanced parameter set we used for the +20.64% backtest only triggers when the market is cleanly trending. In the current sideways regime, those filters block every entry.  
- Swing Grid is stuck on “Insufficient history” because the container isn’t finishing the preload of candles (either the fetch times out or the configured history depth is shorter than the 288-bar requirement), so it never leaves the warm-up guard.

---

## 2. Point-by-Point Answers

### 2.1 Data Source Differences

| What we’re comparing | Backtest | Live container | Why it matters |
| --- | --- | --- | --- |
...
| Warm-up | We load the whole dataset before the first signal | We need ~300 bars on startup for the 288 SMA. If preload fails, the bot waits indefinitely | Explains why Swing Grid keeps reporting “Insufficient history.” |
...
- The only external dependency that can block us is the history fetch during `prepare()`. If the container launches before networking is ready or the API throttles us, the preload fails and the strategy never leaves the warm-up guard.  
...
### Swing Grid (Strategy 2)

1. **History preload failed:** We need at least 300–450 bars before the SMA/ATR calculations make sense. If `fetch_market_snapshot` returns fewer candles or times out, `generate_signal` keeps saying “Insufficient history.”
...
1. **Set env vars before starting the container**
   ```powershell
   # Adaptive Momentum
   $env:BOT_STRATEGY = "adaptive_momentum"
   $env:BOT_STRATEGY_PARAMS = Get-Content configs/enhanced_params.json -Raw

   # Swing Grid
   $env:BOT_HISTORY = "450"
   $env:BOT_STRATEGY_PARAMS = '{"preload_history_bars": 450, "min_history_bars": 288}'
   ```

2. **Confirm preload**
   - Swing Grid should log `preload_bars=450`. If not, relaunch after the feed is ready.

3. **Early signal check**
   - Adaptive Momentum should trade within 6–12 hours once the parameters fit the regime. If not, the hold reasons will show what’s still too tight.

4. **Replay the same window**
   - I’ll add a backtest replay of the same three-day live window so future comparisons line up.
...
| Day | Task | Owner |
| --- | --- | --- |
| Day 1 | Share env vars + log instructions with Ops | Jay |
| Day 2 | Run a controlled live test with the “sideways” profile | Jay & Ops |
| Day 3 | Confirm Swing Grid preload; add retries if needed | Jay |
| Day 4 | Replay the same live window in backtests | Jay |
| Day 5 | Deliver a side-by-side report (live vs replay) | Jay |

---

## 7. Closing

The code is sound; the gap is caused by the market we’re currently trading and the settings used in the containers. By lining up the runtime parameters, ensuring the history preload succeeds, and giving the bot a profile that adapts to sideways conditions, we can bring the live results back in line with the backtests. I’ll execute the plan above within the five-day window and keep you updated as we knock down each step.
